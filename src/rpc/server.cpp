...
UniValue stop(const JSONRPCRequest& jsonRequest)
{
    ...
    if (jsonRequest.fHelp || jsonRequest.params.size() > 1)
        throw std::runtime_error(
            "stop\n"
            "\nStop MobilityCoin server.");
    // Event loop will exit after current HTTP requests have been handled, so
    // this reply will get back to the client.
    StartShutdown();
    return "MobilityCoin server stopping";
}
...
static const CRPCCommand vRPCCommands[] =
    {
        ...
        /* MOBIC features */
        {"mobic", "listmasternodes", &listmasternodes, true },
        {"mobic", "getmasternodecount", &getmasternodecount, true },
        {"mobic", "createmasternodebroadcast", &createmasternodebroadcast, true },
        {"mobic", "decodemasternodebroadcast", &decodemasternodebroadcast, true },
...
{
    return "> mobic-cli " + methodname + " " + args + "\n";
}
...
std::string HelpExampleRpc(std::string methodname, std::string args)
{
    return "> curl --user myusername --data-binary '{\"jsonrpc\": \"1.0\", \"id\":\"curltest\", "
           "\"method\": \"" +
           methodname + "\", \"params\": [" + args + "] }' -H 'content-type: text/plain;' http://127.0.0.1:22488/\n";
}
